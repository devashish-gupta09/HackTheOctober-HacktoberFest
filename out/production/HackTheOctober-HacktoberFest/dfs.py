...
'''
This program takes edge list as input and performs Depth First Search
...
class Graph:
    ...
    def dfs(self, root):
       ...
       nx.draw_planar(G, with_labels=True, arrows=True,
                      node_size=3000, font_size=30, font_color="yellow", width=5)
       plt.show()
       plt.pause(10)
       plt.clf()
       G = nx.Graph()
       ...
       while(stack):
           ...
           print(vertex)
           for edge in self.graph:
               if(edge[0] == vertex and edge[1] not in visited and edge[1] not in stack):
                   stack.append(edge[1])
                   G.add_edges_from([edge,],color="red")
                   nx.draw_planar(G, with_labels=True, arrows=True,
                                  node_size=3000, font_size=30, font_color="yellow", width=5)
                   plt.show()
                   plt.pause(3)
